...
public class ListFeedAdapter extends BaseAdapter {
    ...
    public ListFeedAdapter(Context context, ArrayList<FeedDataItem> list) {
        this.context = context;
        this.feedItemList = list;
        repostImageWidth = context.getResources().getDisplayMetrics().widthPixels;
    }
    ...
    @Override
    public FeedDataItem getItem(int position) {
        return feedItemList != null ? feedItemList.get(position) : null;
    }
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        final FeedDataItem feedItem =getItem(position);
        ViewHolder holder = null;
        if (convertView == null) {
            convertView = LayoutInflater.from(context).inflate(R.layout.feed_list_item, parent,
                    false);
            holder = new ViewHolder();
            holder.llListItemTitle = (RelativeLayout) convertView.findViewById(R.id.rl_title);
            // holder.ivListItemBackground = (ImageView)
            // convertView.findViewById(R.id.iv_feed_list_item_background);
            ...
            holder.tvPublishTime = (TextView) convertView
                    .findViewById(R.id.tv_list_item_publish_time);
            holder.ivReplyPhoto = (ImageView) convertView
                    .findViewById(R.id.iv_list_item_reply_photo);
            ...
            holder.tvReplyCount = (TextView) convertView
                    .findViewById(R.id.tv_list_item_reply_photo_count);
            ...
            RelativeLayout.LayoutParams paramsRepost = (RelativeLayout.LayoutParams) holder.ivPhoto
                    .getLayoutParams();
        ...
        // 显示头像
        ImageLoader.getInstance().displayImage(feedItem.user.head, holder.ivHeader,
        ...
        // 发的图
        ImageLoader.getInstance().displayImage(feedItem.image.url, holder.ivPhoto,
                EmoApplication.options_normal);

        // 回照片
        if (feedItem.is_repost) {
            if (!TextUtils.isEmpty(feedItem.repost.repost_image.url)) {
                ImageLoader.getInstance().displayImage(feedItem.repost.repost_image.url,
                        holder.ivReplyPhoto,
                        EmoApplication.options_normal);
                holder.ivReplyPhoto.setVisibility(View.VISIBLE);
                holder.ivReplyPhoto.setClickable(true);
                RelativeLayout.LayoutParams params = (RelativeLayout.LayoutParams) holder.ivReplyPhoto
                        .getLayoutParams();
    ...
    private void onClickEvent(View view, ViewHolder holder) {
        ...
        switch (id) {
            ...
            case R.id.iv_list_item_reply_photo:
                ...
                EmoRequest request = new EmoStringRequest(AppConfig.ServiceUrl.strGetFeed, headers,
                        json.toString(), new IEmoHttpListener() {
                            @Override
                            public void onSuccess(String tag, String requestUrl,
                                    String requestResult) {
                                if (TextUtils.equals(requestUrl, AppConfig.ServiceUrl.strGetFeed)
                                        && tag == null) {
                            ...
                            @Override
                            public void onFailed(String tag, String requestUrl, int errorNo,
                                    String requestResult) {
    ...
    private void getLikeList(View view, ViewHolder holder) {
        ...
        EmoRequest request = new EmoStringRequest(AppConfig.ServiceUrl.strGetLikerList, headers,
                json.toString(), new IEmoHttpListener() {
                    @Override
    ...
    private void requestLike(View view, ViewHolder holder, boolean isLike) {
        ...
        EmoRequest request = new EmoStringRequest(AppConfig.ServiceUrl.strLikeFeed, headers,
                json.toString(), new IEmoHttpListener() {
                    @Override
                    public void onSuccess(String tag, String requestUrl, String requestResult) {
